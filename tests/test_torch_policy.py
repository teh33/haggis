...
class TorchPolicyTests(unittest.TestCase):
    ...
    def test_ladder_supports_labeled_bot_specs_with_distinct_models(self):
        with tempfile.TemporaryDirectory() as directory:
            data_path = Path(directory) / "records.jsonl"
            model_a = Path(directory) / "a.pt"
            model_b = Path(directory) / "b.pt"
            export_self_play_jsonl(data_path, bot_a="point-aware", bot_b="bomb-control", hands=2, seed=35, observation_mode="player")
            train_torch_policy_from_jsonl(data_path, output_path=model_a, epochs=1, hidden_size=8, batch_size=1, validation_fraction=0.25, seed=4)
            train_torch_policy_from_jsonl(data_path, output_path=model_b, epochs=1, hidden_size=8, batch_size=1, validation_fraction=0.25, seed=5)

            result = run_ladder((f"current:torch-policy@{model_a}", f"candidate:torch-policy@{model_b}"), hands_per_match=1, seed=7)

        self.assertEqual({entry.bot for entry in result.entries}, {"current", "candidate"})
        self.assertEqual({match.bot_a for match in result.matches} | {match.bot_b for match in result.matches}, {"current", "candidate"})
